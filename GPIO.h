...
namespace exploringBB { // This helps to clarify the adress of the variables that will be used in BB

typedef int (*CallbackType)(int);
enum GPIO_DIRECTION{ INPUT, OUTPUT };// Input = 0; Output = 1;
enum GPIO_VALUE{ LOW=0, HIGH=1 };
enum GPIO_EDGE{ NONE, RISING, FALLING, BOTH };// Pound define
...
private:
	int write(string path, string filename, string value);// Overloaded function 
	int write(string path, string filename, int value);// They have the same name but different inputs
